...
install-local: build
    ...
    vsce package --no-dependencies --allow-missing-repository -o "$root/vscode-naproche.vsix"
    code --install-extension "$root/vscode-naproche.vsix" --force
    echo "Installed vscode-naproche.vsix — reload VS Code to activate"
...
publish-dry: build
    cd packages/extension && vsce ls --no-dependencies --allow-unused-files-pattern --allow-missing-repository
